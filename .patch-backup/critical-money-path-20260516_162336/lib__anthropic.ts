    ...
    if (toolUseBlocks.length > 0) {
      ...
      if (quoteResultData) {
        ...
        if (quoteResultData.blocked) {
          ...
          const budgetUsd = Number(quoteResultData.fromAmountUsd ?? quoteResultData.amount);
          const needsApproval = Boolean(quoteResultData.needsApproval);
          const approveAmount = quoteResultData.approveAmount ? String(quoteResultData.approveAmount) : undefined;
          const spender = quoteResultData.spender ? String(quoteResultData.spender) : undefined;
          const routerAddress = quoteResultData.routerAddress ? String(quoteResultData.routerAddress) : undefined;
          const approvalId = await createApproval(String(quoteResultData.toAddress), String(quoteBlockInput!.chain), budgetUsd, slippage, walletAddress, quoteResultData.fromToken ? String(quoteResultData.fromToken) : undefined, routerAddress, needsApproval || undefined, approveAmount, spender);
          pipelineData = {
            type: "quote",
            ...
            approveTxData: (quoteResultData.approveTxData as Record<string, unknown>) ?? undefined,
          };
        }
