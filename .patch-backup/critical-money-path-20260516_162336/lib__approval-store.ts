...
// ─── APPROVAL_SECRET HMAC signing ─────────────────────────────────────────
function signApprovalId(rawId: string): string {
  const secret = process.env.APPROVAL_SECRET;
  if (!secret) return rawId;
  const sig = createHmac("sha256", secret).update(rawId).digest("hex").slice(0, 16);
  return `${rawId}.${sig}`;
}
function verifyAndExtractId(signedId: string): { rawId: string; valid: boolean } {
  const secret = process.env.APPROVAL_SECRET;
  if (!secret) return { rawId: signedId, valid: true };
  const dot = signedId.lastIndexOf(".");
  if (dot === -1) return { rawId: signedId, valid: false };
  const rawId = signedId.slice(0, dot);
  const expected = createHmac("sha256", secret).update(rawId).digest("hex").slice(0, 16);
  return { rawId, valid: signedId.slice(dot + 1) === expected };
}
...
export async function createApproval(
  ...
  return signApprovalId(id);
}

export async function validateAndConsumeApproval(signedId: string): Promise<{ valid: boolean; reason?: string; approval?: Approval; code?: "missing" | "replay" | "expired" }> {
  if (typeof global !== "undefined" && (global as any).__mockValidateAndConsumeApproval) {
    return (global as any).__mockValidateAndConsumeApproval(signedId);
  }

  // Verify HMAC signature before any Redis/memory lookup
  const { rawId: id, valid: sigValid } = verifyAndExtractId(signedId);
  if (!sigValid) return { valid: false, reason: "Approval ID signature invalid.", code: "missing" };
