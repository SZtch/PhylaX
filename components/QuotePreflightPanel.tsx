...
export function QuotePreflightPanel({ quote, error, fromSymbol, quoteSource }: Props) {
  if (error) {
    return (
      <motion.div 
        ...
        className="bg-white/60 backdrop-blur border border-destructive/30 rounded-3xl overflow-hidden shadow-sm"
      >
  ...
  return (
    ...
    >
      <div className="px-5 sm:px-6 py-5 border-b border-border/50 flex flex-col sm:flex-row sm:items-center justify-between bg-white/40 gap-4">
      ...
      <div className="p-5 sm:p-6 relative">
        {/* Route Visualizer */}
        {fromSymbol && (
          <div className="flex items-center justify-center gap-4 mb-8 bg-white/60 backdrop-blur p-4 rounded-xl border border-border relative overflow-hidden">
            <div className="px-4 py-2 bg-white rounded-lg text-foreground font-bold border border-border shadow-sm relative z-10">
              {fromSymbol}
            </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          <div className="bg-white/60 backdrop-blur p-4 rounded-xl border border-border flex flex-col justify-between group hover:border-electric/30 transition-colors">
            <span className="text-xs font-bold text-muted-foreground uppercase tracking-widest mb-2">Expected Out</span>
            <span className="text-xl font-bold text-emerald-600">${quote.expectedOutputUsd.toFixed(4)}</span>
          </div>
          <div className="bg-white/60 backdrop-blur p-4 rounded-xl border border-border flex flex-col justify-between group hover:border-electric/30 transition-colors">
            <span className="text-xs font-bold text-muted-foreground uppercase tracking-widest mb-2">Price Impact</span>
            <span className={`text-xl font-bold ${quote.slippage > 2 ? "text-destructive" : "text-amber-500"}`}>
              {quote.slippage.toFixed(2)}%
            </span>
          </div>
          <div className="bg-white/60 backdrop-blur p-4 rounded-xl border border-border flex flex-col justify-between group hover:border-electric/30 transition-colors">
            <span className="text-xs font-bold text-muted-foreground uppercase tracking-widest mb-2">Est. Gas</span>
            <span className="text-xl font-bold text-foreground">${quote.gasFeeUsd.toFixed(4)}</span>
          </div>
          <div className="bg-white/60 backdrop-blur p-4 rounded-xl border border-border flex flex-col justify-between group hover:border-electric/30 transition-colors">
            <span className="text-xs font-bold text-muted-foreground uppercase tracking-widest mb-2">Route</span>
            <span className="text-sm font-bold text-foreground/80 truncate" title={quote.route}>{quote.route}</span>
          </div>
        </div>

        <div className="bg-emerald-50 border border-emerald-100 text-emerald-700 px-4 py-3 rounded-xl text-sm flex items-start gap-3">
